...
class ConservativeTradingTest:
    ...
    async def get_live_eth_price(self):
        """Get live ETH price from Kraken REST API."""
        print(f"\n📊 GETTING LIVE ETH PRICE")
        print("-" * 50)
        
        try:
            # Try to get live price via REST API
            import aiohttp
    ...
    async def prepare_minimal_order_test(self):
        ...
        try:
            ...
            if eth_amount >= min_eth_order:
                ...
                print(f"   💡 May need to increase order value to ~${min_eth_order * self.current_eth_price:.2f}")
