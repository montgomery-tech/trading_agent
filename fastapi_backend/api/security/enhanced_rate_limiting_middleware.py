...
class RateLimitingConfigMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_reset_request(self, request: Request) -> Response:
        ...
        try:
            ...
            # Parse reset parameters from URL path
            path_parts = request.url.path.split("/")
            if len(path_parts) >= 5:
                reset_type = path_parts[4]  # user or ip

                if reset_type == "user" and len(path_parts) >= 6:
                    user_id = path_parts[5]
                    self.rate_limiting_service.reset_user_limits(user_id)
                    return JSONResponse(content={
                        "success": True,
                        "message": f"Reset rate limits for user {user_id}"
                    })

                elif reset_type == "ip" and len(path_parts) >= 6:
                    ip = path_parts[5]
                    self.rate_limiting_service.reset_ip_limits(ip)
                    return JSONResponse(content={
                        "success": True,
                        "message": f"Reset rate limits for IP {ip}"
                    })
            ...
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": "Failed to handle reset request"
                }
            )

    async def _extract_user_from_request(self, request: Request) -> Optional[Dict]:
        """Extract user data from request (same as EnhancedRateLimitingMiddleware)."""
        try:
            auth_header = request.headers.get("Authorization")
            if not auth_header or not auth_header.startswith("Bearer "):
                return None
            ...
            logger.debug(f"Could not extract user data: {e}")
