...
# AWS FastAPI Deployment Verification Script
...
print_status "🔍 AWS FastAPI Deployment Verification" $BLUE
print_status "=======================================" $BLUE
...
print_step "Step 2: Testing SSH Connectivity"

if ssh -i trading-api-keypair.pem -o StrictHostKeyChecking=no -o ConnectTimeout=10 ec2-user@${PUBLIC_IP} "echo 'SSH connection successful'" 2>/dev/null; then
    print_status "✅ SSH connectivity verified" $GREEN
else
    print_status "❌ SSH connection failed" $RED
    exit 1
fi

# Step 3: Verify Application Status on EC2
print_step "Step 3: Verifying Application Status on EC2"
...
python3 -c "
import sys
deps = ['fastapi', 'uvicorn', 'sqlalchemy', 'psycopg2', 'pydantic', 'decouple']
for dep in deps:
    try:
        __import__(dep)
        print(f'✅ {dep}')
...
if [ -f "/var/log/trading-api/app.log" ]; then
    ...
    echo "⚠️  Log file not found at /var/log/trading-api/app.log"
fi
EOF
...
print_status "🧪 Testing internal health check (localhost:8000)..." $YELLOW
INTERNAL_HEALTH=$(ssh -i trading-api-keypair.pem -o StrictHostKeyChecking=no ec2-user@${PUBLIC_IP} "curl -f -s --connect-timeout 5 http://localhost:8000/health >/dev/null 2>&1" && echo "PASS" || echo "FAIL")
...
if [ "$EXTERNAL_HEALTH" = "PASS" ]; then
    ...
    ssh -i trading-api-keypair.pem -o StrictHostKeyChecking=no ec2-user@${PUBLIC_IP} "netstat -tulpn | grep :8000 || ss -tulpn | grep :8000"
fi
...
# Step 7: Database Connectivity Test
print_step "Step 7: Testing Database Connectivity"
...
try:
    ...
    print(f'✅ Environment loaded, DATABASE_URL configured')
...
APP_PROCESSES=$(ssh -i trading-api-keypair.pem -o StrictHostKeyChecking=no ec2-user@${PUBLIC_IP} "ps aux | grep uvicorn | grep -v grep | wc -l" 2>/dev/null)
...
echo "   🏠 Internal Health: $([ "$INTERNAL_HEALTH" = "PASS" ] && echo "✅ Working" || echo "❌ Failed")"
...
AWS FastAPI Deployment Verification Results
Generated: $(date)
==========================================
...
- Internal Health: $INTERNAL_HEALTH
